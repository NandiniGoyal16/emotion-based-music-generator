...
class SACAgent:
    ...
    def _soft_update_policy(self, state):
        ...
        # Exponentiate Q values scaled by alpha
    ...
    def train_from_data(self, instrument_features, emotion, episodes=100):
        ...
        for episode in range(episodes):
            ...
            # Simulate a few steps for trajectory
            for _ in range(10):
                ...
                # Next state (simplified assumption: separate independent decisions or sequential)
                # Here we treat next note as next state
                next_state = action
...
def synthesize_note_v2(freq, duration, instrument, sr=22050):
    ...
    if instrument in ["flute", "bansuri"]:
        ...
        # Sarod has a punchier attack and deeper resonance than sitar
        ...
        # Santoor is resonant and metallic (hammered)
        for n in [1, 2, 3, 5, 8]:
            audio += (0.5 / n) * np.sin(2 * np.pi * n * freq * t)
        audio += 0.1 * np.sin(2 * np.pi * 12 * freq * t) # High sparkle
        envelope = np.exp(-4 * t) * (1 + 0.05 * np.sin(2 * np.pi * 15 * t)) # Slight tremolo effect
...
def generate_session(emotion, instrument, data_handler=None, duration=10, alpha=0.2, episodes=100):
    features = {'tempo': 90}
    if data_handler:
        features = data_handler.get_instrument_features(instrument)
        print(f"📊 Features: {features}")

    # Initialize SAC Agent instead of TRPO
    agent = SACAgent(state_size=7, action_size=7, alpha=alpha)
    print(f"🧠 Training SAC Agent for {emotion} (Alpha: {alpha}, Episodes: {episodes})...")
    agent.train_from_data(features, emotion, episodes=episodes)
    
    target_tempo = features['tempo']
    # Adjust for all emotion contexts
    if emotion in ['sad', 'fear']: target_tempo *= 0.8
    elif emotion in ['happy', 'surprise', 'angry']: target_tempo *= 1.2
    elif emotion in ['romantic']: target_tempo *= 0.9
    elif emotion in ['calm', 'neutral']: target_tempo *= 0.75
    elif emotion in ['disgusted']: target_tempo *= 0.85
    else: target_tempo *= 1.0

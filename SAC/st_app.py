...
st.set_page_config(
    page_title="Advanced Emotion Music Generator (CNN + SAC)",
    page_icon="🧠",
    layout="wide"
)
...
st.markdown('<div class="sub-header">Powered by Keras CNN & SAC Reinforcement Learning</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.divider()
    st.write("🔧 **SAC Brain Tuning**")
    sac_alpha = st.slider("SAC Temperature (Alpha)", 0.01, 1.0, 0.2, help="Higher alpha = more exploration/randomness, lower = more exploitation/structure.")
    sac_episodes = st.slider("Training Episodes", 10, 500, 100, step=10)
...
with col1:
    st.info("Click to run the SAC Agent.")
    if st.button("🚀 Generate with AI", type="primary"):
        with st.spinner(f"SAC Agent generating {selected_instrument} melody..."):
...
if 'audio' in st.session_state:
    ...
    with col2:
        ...
        with tab2:
            ...
            ax2.set_title("SAC Policy Matrix (State-Action probs)")
